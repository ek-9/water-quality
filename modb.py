...
def get_data_from_db():
    ...
    with conn.cursor() as cursor:
        ...
        sql = "SELECT do FROM sensor_data ORDER BY datetime DESC LIMIT 288"
        cursor.execute(sql)
        result = cursor.fetchall()

        for row in result:
            do_values.append(result[0][0])

    data = {
        # 'DO': [6 for i in range(288)]
        'DO': do_values
        # 'DO' : [3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,]
    }
...
def get_temp_from_db():
    ...
    with conn.cursor() as cursor:
        ...
        sql = "SELECT temp FROM sensor_data ORDER BY datetime DESC LIMIT 288"
        cursor.execute(sql)
        result = cursor.fetchall()

        for row in result:
            temp_values.append(result[0][0])
    ...
    try:
        ...
        while True:
            try :
                ...
                # last_predicted_quality = dl_.prediction(input_data)
                # cur = conn.cursor()
                # cur.execute("INSERT INTO predicted_data VALUES (%s)", (last_predicted_quality[0]))
                #
                # conn.commit()

                # ----------------------------------------------------------------------------------------------------------------
                # control
                temp_data = get_temp_from_db()
                # PumpControl.control(input_data.iloc[-1, 0], temp_data.iloc[-1, 0])
